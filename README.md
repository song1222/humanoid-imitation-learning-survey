...
Different from a general humanoid-robot-learning list, this repository keeps one main classification line: **how a method turns demonstrations or motion priors into executable humanoid behavior**. Data source, simulator, robot platform, project links, and code links are stored as metadata in `data/papers.json`.
...
The canonical data file is [`data/papers.json`](data/papers.json). Each entry keeps the full paper title, method name, venue, year, data source, primary category, platform, and links.
